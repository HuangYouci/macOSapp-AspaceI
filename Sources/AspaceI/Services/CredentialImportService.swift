...
final class CredentialImportService: Sendable {
    ...
    func importAvailable(homeDirectory: URL = FileManager.default.homeDirectoryForCurrentUser, includeKeychain: Bool = true, platforms: Set<PlatformKind> = Set(PlatformKind.allCases)) -> [ImportedCredential] {
        ...
        return [
            ...
            platforms.contains(.antigravity) ? importedAntigravity(tokenFile: antigravityToken, database: antigravityDatabase, includeKeychain: includeKeychain) : nil
        ].compactMap { $0 }
    }
    ...
    /// Antigravity 2.0 起登入放在登入 Keychain，但只在使用者主動按「讀取這台 Mac」時才讀：
    /// 官方 App 每次自己換 token 都會把項目重寫成只信任它自己，背景輪詢去讀就會每五分鐘跳一次
    /// 密碼框。背景輪詢退回 jetski 檔案，它可能停在舊帳號，但那是可以接受的代價。
    private func importedAntigravity(tokenFile: URL, database: URL, includeKeychain: Bool) -> ImportedCredential? {
        // 背景輪詢完全不呼叫，不是靠 kSecUseAuthenticationUISkip 擋——那個旗標能不能壓掉舊式
        // ACL 的密碼框沒有把握，而這裡猜錯的代價是每五分鐘跳一次視窗。
        if includeKeychain, let data = AntigravitySystemCredentialService.shared.load(allowPrompt: true), !data.isEmpty {
